...
@pytest.mark.parametrize(
    ("quantization", "weight_nz_mode", "is_310p_device", "expected"),
    [
        ("ascend", 0, False, True),
        (None, 2, False, True),
        (None, 0, True, True),
...
def test_rfork_processed_layout_covers_quantization_and_nz_modes(
    monkeypatch, quantization, weight_nz_mode, is_310p_device, expected
):
    monkeypatch.setattr(
        "vllm_ascend.model_loader.rfork.rfork_loader.get_ascend_config",
    ...
    monkeypatch.setattr(
        "vllm_ascend.model_loader.rfork.rfork_loader.is_310p",
        lambda: is_310p_device,
    )
    loader = RForkModelLoader(DummyLoadConfig({}))
...
def test_rfork_fallback_restores_rc1_moe_counter(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        SimpleNamespace(AscendMoERunner=_FakeAscendMoERunner),
    )
    vllm_config = SimpleNamespace(compilation_config=SimpleNamespace())
    snapshot = _snapshot_process_global_model_state(vllm_config)
    _FakeAscendMoERunner.moe_counter = 5

    _reset_process_global_model_state(vllm_config, snapshot=snapshot)

    assert _FakeAscendMoERunner.moe_counter == 4
